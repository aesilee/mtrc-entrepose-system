...
export default function CompleteFollowUpModal({ followUpId, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    try {
      await api.put(`/follow-ups/${followUpId}/complete`, { completedRemarks: remarks });
      onSaved();
    } finally {
      setSaving(false);
    }
    ...
    <div style={styles.backdrop} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <div style={styles.title}>Complete Follow-up</div>
          <button type="button" style={styles.closeBtn} onClick={onClose}>×</button>
        </div>
        <form onSubmit={handleSubmit} style={styles.body}>
          <label style={styles.label}>
            Remarks
            <textarea style={{ ...styles.input, minHeight: 80 }} value={remarks} onChange={(e) => setRemarks(e.target.value)} />
          </label>
          <div style={styles.footer}>
            <button type="button" style={styles.secondaryBtn} onClick={onClose}>Cancel</button>
            <button type="submit" style={styles.primaryBtn} disabled={saving}>{saving ? "Saving…" : "Mark complete"}</button>
          </div>
        </form>
      </div>
    </div>
